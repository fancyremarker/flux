...
describe 'Flux' do
  ...
  describe "following" do
    ...
    it "doesn't add duplicates to the followed list even if the event fires multiple times" do
      ...
      10.times do
        post "/events/#{@schema_id}", [['client:gravity:action:follow:user', {follower: 'user2', followee: 'user1'}]].to_json
        post "/events/#{@schema_id}", [['client:gravity:action:follow:user', {follower: 'user3', followee: 'user1'}]].to_json
      end
      get "/query?keys[]=user1:followers&maxResults=10"
  ...
  describe "unfollowing" do
    ...
    it "updates followers on the unfollowed user" do
      post "/events/#{@schema_id}", [['client:gravity:action:unfollow:user', {follower: 'user3', followee: 'user1'}]].to_json
      get "/query?keys[]=user1:followers&maxResults=10"
  ...
  describe "posting" do
    before(:each) do
      ...
      post "/events/#{@schema_id}", [['client:gravity:action:follow:user', {follower: 'user2', followee: 'user1'}],
    ...
    it "updates the feed of all users following the poster" do
      post "/events/#{@schema_id}", [['client:gravity:action:post', {user: 'user1', post: 'post1', '@targets' => ['[user].followers.feedItems'], '@add' => 'post'}]].to_json
    ...
    it "sends a sequence of posts to the correct subscribers" do
      4.times do |i| 
        post "/events/#{@schema_id}", 
             [['client:gravity:action:post', {user: "user#{i+1}", post: "post#{i+1}", '@targets' => ['[user].followers.feedItems'], '@add' => 'post'}]].to_json
      end
